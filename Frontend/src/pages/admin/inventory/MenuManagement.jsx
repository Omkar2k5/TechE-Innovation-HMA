    ...
    <div>
      ...
      <div className="bg-white p-4 rounded shadow">
        <table className="w-full text-left">
          <thead><tr className="text-sm text-gray-600"><th>Name</th><th>Category</th><th>Price (₹)</th><th>Cost (₹)</th><th>Actions</th></tr></thead>
          <tbody>
            {menus.map(m=> (
              <tr key={m._id} className="border-t">
                ...
                <td className="py-2">₹{(m.ingredients||[]).reduce((s,ingName)=> {
                  const ingredient = items.find(i=>i.name.toLowerCase() === ingName.toLowerCase())
                  return s + (ingredient?.costPerUnit || 0)
                },0).toFixed(2)}</td>
                <td className="py-2">
                  <button onClick={()=>setEditing(m) || setShowAdd(true)} className="px-2 py-1 mr-2 bg-yellow-600 text-white rounded">Edit</button>
                  <button onClick={()=>onDeleteMenu && onDeleteMenu(m.itemId || m._id)} className="px-2 py-1 bg-red-600 text-white rounded">Delete</button>
                </td>
              </tr>
            ))}
          </tbody>
