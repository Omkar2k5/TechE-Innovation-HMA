...
// @desc    Get previous orders for a table (unpaid orders - for manager's previous orders view)
// @route   GET /api/orders/table/:tableId/previous
// @access  Private (Receptionist, Manager, Owner)
router.get('/table/:tableId/previous', protect, authorize('receptionist', 'manager', 'owner'), async (req, res) => {
  try {
    const { tableId } = req.params;
    const hotelId = req.user.hotelId;

    console.log('🔍 Fetching previous orders for table:', { hotelId, tableId });

    // Find the order document
    const orderDocument = await Order.findById(hotelId);

    if (!orderDocument) {
      console.log('📝 No order document found for hotel:', hotelId);
      return res.status(200).json({
        success: true,
    ...
    // Find the bill document to check payment status
    const billDocument = await Bill.findById(hotelId);

    // Find orders for the specific table that are active and not served/cancelled
    ...
    console.log(`📊 Found ${tableOrders.length} active orders for table ${tableId}`);

    // Filter out orders that have been paid
    // Default behavior: Include all active orders unless we can confirm bill is PAID
    const previousOrders = tableOrders.filter(order => {
      // If no bill document exists, include all orders
      if (!billDocument) {
    ...
    res.status(500).json({
      success: false,
      message: 'Server error occurred while fetching previous orders',
      error: process.env.NODE_ENV === 'development' ? error.message : 'Internal server error'
    });
  }
});
